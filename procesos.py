        ...
        if procesos[i].id == id:
            ...
            if value > 0:
                if actual + value > MAX_SIZE:
                    print("No se puede agrandar, demasiado grande")
                    return 0
                try:
                    if i+1 == len(procesos):
                        if procesos[i-1].id == '@':
                        ...
                        if tam+value <=MAX_SIZE:
                            ...
                            print("No se pudo incrementar el proceso, no cabe")
                            return 0
                    if procesos[i-1].id == '@':
                        if procesos[i-1].size == value:
                            procesos[i].size += value
                    ...
                    if procesos[i+1].id == '@': 
                        if procesos[i+1].size == value:
                            ...
                            procesos[i].size += value
                            return value
                    if procesos[i+1].id == '@' and procesos[i-1].id == '@':
                        tamJ = procesos[i+1].size + procesos[i-1].size
                        if tamJ == value:
                            procesos[i].size += value
                            procesos.pop(i+1)
                            procesos.pop(i-1)
                            ...
                            procesos[i+1].size -= value - procesos[i-1].size
                            procesos[i].size += value
                            procesos.pop(i-1)
                            return value
                    i = 0
                    while i < len(procesos)-1:
                        if procesos[i].id == id:
                            procesos.append(procesos.pop(i))
                        elif procesos[i].id == '@':
                            procesos.pop(i)
                        else:
                            i += 1
                    procesos[len(procesos)-1].size += value
                    return value
                except:
                    print("Excepcion lanzada, No se pudo aumentar el tamaño")
                return 0    
            elif value < 0:
